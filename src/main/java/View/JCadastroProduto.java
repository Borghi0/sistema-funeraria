...
public class JCadastroProduto extends javax.swing.JFrame {
    ...
    private void cadastrar(){
        ...
        if (cxCadQuant.getText().isEmpty() || cxCadPreco.getText().isEmpty() || nome.isEmpty()) {
             JOptionPane.showMessageDialog(
                null,
                "Campos 'Nome', 'Quantidade' e/ou 'Preço' não podem estar vazios.",
                "Erro de Validação",
        ...
        try {
            quantidade = Integer.parseInt(cxCadQuant.getText());
            preco = Double.parseDouble(cxCadPreco.getText());
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(
                null,
                "Erro ao converter Quantidade e/ou Preço. Certifique-se de que são números inteiros/doubles válidos.",
                "Erro de Formato",
                JOptionPane.ERROR_MESSAGE
            );
            return;
        ...
        try {
            if (quantidade <= 0) {
                throw new IllegalArgumentException("A quantidade do produto deve ser um valor positivo (maior que zero).");
            }
            if (preco <= 0) {
                throw new IllegalArgumentException("O preço do produto deve ser um valor positivo (maior que zero).");
            }
